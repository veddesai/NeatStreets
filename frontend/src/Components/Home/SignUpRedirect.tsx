...
export const SignUpRedirect: React.FC = () => {
  ...
  user?.role === "END_USER" ? (
    ...
    <div className="text-center">
      ...
      <p className="my-20 text-xl">
        You are currently assigned to clear {user?.assignedPosts.length}
        &nbsp;reports.
      </p>

      <div className="bg-blue-800 dark:bg-yellow-500 my-10 max-lg:mx-8 lg:mx-96 text-2xl text-white px-6 py-2">
        <Link to={"/posts"}>
          View others litter reports or mark them as completed.
          {/* Left here, many things left to do based on role ..soon */}
        </Link>
      </div>
    </div>
  )
